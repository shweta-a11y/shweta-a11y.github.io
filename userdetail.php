...
if(isset($_POST['submit']))
{
    ...
    $sql = "SELECT * from view_customer where ID=$from";
    $query = mysqli_query($conn,$sql);
    $sql1 = mysqli_fetch_array($query); // returns array or output of user from which the amount is to be transferred.

    $sql = "SELECT * from view_customer where ID=$to";
    $query = mysqli_query($conn,$sql);
	...
	<div class="container">
        <h2 class="text-center pt-4">Transaction</h2>
            <?php
                include 'db_connection.php';
                $sid=$_GET['id'];
                $sql = "SELECT * FROM  view_customer where id=$sid";
        ...
        <br>
            ...
            <button class="btn mt-3" name="submit" type="submit" id="myBtn" style="background-color:#613f3f">Transfer</button>
            </div>
        </form>
    </div>
